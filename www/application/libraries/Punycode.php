...
class Punycode
{
     ...
     */
    private function ordUTF8($c, $index = 0, &$bytes = null)
    {
        $len = strlen($c);
        $bytes = 0;
        if ($index >= $len)
     ...
     */
    public function EncodePunycodeIDN($value)
    {
        /* search subdomains */
        ...
        if (count($sub_domain) > 1) {
            ...
            foreach ($sub_domain as $sub_value) {
                $sub_result .= '.' . $this->EncodePunycodeIDN($sub_value);
            }
        ...
        while ($h < $input_len) {
            ...
            for ($i = 0; $i < $input_len; ++$i) {
                $ord_input[$i] = $this->ordUtf8($input[$i]);
     ...
     * @link http://belousovv.ru/myscript/phpIDN
     */
    public function DecodePunycodeIDN($value)
    {
        /* search subdomains */
        $sub_domain = explode('.', $value);
        if (count($sub_domain) > 1) {
            $sub_result = '';
            foreach ($sub_domain as $sub_value) {
                $sub_result .= '.' . $this->DecodePunycodeIDN($sub_value);
            }
            return substr($sub_result, 1);
        }
