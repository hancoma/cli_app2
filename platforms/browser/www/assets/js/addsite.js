...
$(function(){
	...
	$(document).on("click", "[cb-action='check']", function(){
		...
		if( !reg_domain || reg_domain == ""){ //빈 값
            ...
            makeModalAlert('Notice', c2ms('값을 입력하세여'));
			return false;

		} else if( !confirmValidUrl(reg_domain) ){

            makeModalAlert('Notice', c2ms('도메인 형식이 아니에여'));
			return false;
		} else {
			...
			$.ajax({
				...
				success:function(res){ console.log(res);
					...
					else
					{
						...
						using_https = res.using_https; //console.log(res);
						...
						$("#cancel_url").click(function(){ 
							...
							$lb_site_url.find(".addsite_status_img").html(""); //label 이미지 초기화
						...
						$lb_site_url.find(".addsite_status_img").html("<img src='/assets/img/addsite/check.png'/>");
	...
	/**********************
	 * SSL 사용 여부 물어보기
	 * http 만 사용하면 SSL 물어보지 않고 바로 다음 단계 이동
	 **********************/ 
	$(document).on("click", "[cb-action='add']" ,function(){ 
		...
		//ip 유효성 검사
		if(!checkIp($("[cb-action='addsite_ip']").val())){
			makeModalAlert('Notice', c2ms('W_notIp'));
			return false;
		}
		...
		{
			$("#modal_email").modal({'backdrop':'static'}).modal('show');
			...
			var verify_ssl = $.ajax({
	            ...
	            beforeSend: function(){
					...
					// SSL 사용 안함 누른 경우
					$("#btn_beta_not_use_ssl").click(function(){
						verify_ssl.abort(); // email 가져오는 ajax 연결 끊기
						$("form[name='reg_form']")
							.attr("action", "/domain/register/register_setdomain")
							.submit();
					});
